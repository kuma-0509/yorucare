...
支援先のデータは `src/lib/support/` のアダプター層から読み込みます。実URLとデータ形式、および掲載元の利用条件が未確認のため、現在はローカルJSONのデモデータで動かしています。デモデータは電話番号とURLを持たず、画面上で「デモデータ」と明示します。

すべての外部データは `DataSource`（提供者 / データセット名 / 更新日 / 参照URL / デモか実データか / **利用の根拠** / ライセンス）を持ち、ダイアログ内の「データについて」で開示します。更新日が確認できない場合は、利用前に公式サイトか電話で確認するよう案内します。

実データは、掲載元のライセンスか個別の許諾が確認できたものだけを表示します。この方針は運用ルールではなく実装で担保していて、`permissionBasis` は省略できない必須フィールドで、根拠が未確認のデータはアダプターが読み込み自体を拒否します。方針と接続前の確認手順は [docs/open-data-licensing.md](docs/open-data-licensing.md) を参照してください。
