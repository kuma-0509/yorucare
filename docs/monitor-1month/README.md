...
node build-pdf.mjs                 # PDF 2種類を出力（はみ出していたら exit 1）
node verify-pdf.mjs                # 出したPDFを読み、必須の文言が入っているか確認
node verify-qr.mjs                 # チラシ上のQRが読めるか確認
```

**`verify-pdf.mjs` は必ず通してください。** 組んだHTMLが正しくても、PDFにしたときに
文字が落ちることがあります（実際に、裏面が表面のCSSで組まれて崩れ、注記の最終行が
消えたまま出ていました）。このスクリプトは出来上がったPDFの文字を読み取って確かめます。

PDFは面ごとに1ページずつ作り、最後に1つに綴じています。1つのHTMLに表と裏を並べると、
どちらか一方のCSSしか効かず、もう一方が崩れるためです。
...
- チラシ裏面の「アプリに書いた内容はご自身の端末の中に保存され、運営者も原則として見ることはできません」は、
  同意説明書をクラウド保存に変えたため現状と合っていない
- `line-messages.html` も同じく端末保存のままで、同意説明書と矛盾している
