...
日付: 2026-09-15
担当チャット: 日次自動開発（再認証だけの導線）

## 今回実装したタスク

再認証だけを行う導線がなく、10分を過ぎると入り直しになる。設定画面は実装済みだったが、停止・退会が `403 reauth_required` のとき `/cloud-login` へ送ると「ログイン済みです」とログアウトしか出なかった。

### 何をしたか

1. `/cloud-login?reauth=1` で、ログイン済みならログアウトせずメール入力へ進む。
2. ログイン済み画面にも「本人確認をやり直す」を置いた。やめるとログイン済み表示に戻る。ログアウトは呼ばない。
3. 設定画面の再認証リンクをその経路へ変えた。
4. 未ログインで `?reauth=1` を開いたときは「ログインは続いています」と案内しない。
5. コード送信・検証は既存の許可リスト照合のまま。削除APIの10分判定は変えていない。
...
- `pnpm test`: 成功（74 files / 772 tests）
- `pnpm build`: 成功

### 次にやること

1. **YC-UX-STORAGE-NOTICES** は PR #64 が未マージ。再実装しない。
2. YC-UX-NORMAL-ENTRY は運営者の移行判断が先。
3. YC-MON-REQUIRED-CHECKS は担当者の GitHub 設定が必要。
4. Preview実機で、再認証後に停止・退会が通ることを確認する（新しいセッションの `createdAt` が10分判定を満たす前提）。
5. 認証メールのキャリア到達確認、Managed Better Auth のGA再確認。

### 引き継ぎ事項

1. **YC-UX-RECORDS-TABLE-ROW を勝手に実装しない。** 運営判断は「記録のみ。いまは開発しない」。
2. 再認証はログアウトしない。`cloudAuthClient.signOut()` を再認証の開始で呼ばないこと。
3. 「使えるか」の判定は `fetchCloudAuthOutcome` / `GET /api/cloud/session` を正とする。
4. 入口フラグは開けていない。本番の到達条件は変えていない。
...
日付: 2026-09-15
担当チャット: 記録一覧表の操作・行高の改善要望を管理表へ記録（開発なし）／参考表の見た目を言語化して更新

## 今回記録したタスク

「これまで」の直近7日表で、日付列の操作ボタンが行を高くし、一覧として見渡しにくい（YC-UX-RECORDS-TABLE-ROW）。

**開発せず、記録だけ残した。いまは実装しない。**

### 要望の内容

1. 「この日の記録をつける」「詳しく見る」「編集する」のボタンは不要。
2. 表の行を選ぶ（タップ／クリック）とその日の記録を編集できる仕様にする。未記録の日は新規入力、記録済みの日は編集へ進む想定。
3. 行の高さは表計算ソフトのように文章量へ合わせ、未記録行を狭くして直近7日を一度に見渡せるようにする。

### 参考表の見た目（言語化）

運営が共有した参考は表計算ソフトの一覧表。**列の項目名や中身は真似しない。** 現行の日付・気分・睡眠・メモなど、記録表の列は維持する。真似するのは密度と行高のルールだけ。

- 1画面に、見出し行の下へデータ行が隙間なく並ぶ。行の中にボタン、リンク、操作文言は無い。見えるのは見出し・日付・各セルの値だけである
- 空欄や短い値（記号、1語、時刻のような1行で収まる内容）の行は、文字1行分の高さにとどまる。行どうしの上下余白は小さく、同じ画面に多くの行が見える
- セル内の文章が増えた行だけが、その文章の行数に合わせて高くなる。折り返しはセル幅の中で行い、隣の短いセルも同じ行高になる
- セルは細い罫線でマス目として区切られる。見出し行は背景や太字でデータ行と区別し、データ行は縞模様やカード化をせず同じルールで並ぶ
- 角丸の大きなカードや、日付列へ操作を積み重ねるレイアウトにはしない
- 文字サイズは通常の本文程度とし、行数を稼ぐために極端に小さくしない
- 目的は、直近7日を少ないスクロールで見渡せるようにすること

### 現行実装（着手時に見る場所）

- 表本体: `src/components/records/records-table.tsx`（日付列に操作ボタン、`min-h-11`）
- 文言: `src/lib/copy.ts` の `recordsList.addRecord` / `viewDetail` / `edit`
- 表示テスト: `src/components/tabs/records-tab.test.tsx`（ボタン到達を固定）
- 実機チェック: `docs/smartphone-test-checklist.md` の D-2〜D-6、D-8b（詳しく見るから削除する経路）
- 置き換える完了課題: 2026-09-10「記録一覧が1日ごとのカードで、日付を横断して見比べにくい」（ボタン到達を契約にした）

### 着手時に決めること

詳しく見るダイアログは削除導線も兼ねている。ボタンを外すなら、1件削除の到達経路を先に決める。行を狭くしても、支援技術とタッチ操作で行選択できることを表示テストで固定する。

### 検証結果

文書のみのため lint / test / build は未実施。

### 次にやること

この課題は未着手のまま残し、今回は開発しない。クラウド保存の残作業と既存の未着手課題の優先順位は変えない。参考表の列項目は真似せず、上の「参考表の見た目（言語化）」を密度の正とする。

### 引き継ぎ事項

1. **YC-UX-RECORDS-TABLE-ROW を勝手に実装しない。** 運営判断は「記録のみ。いまは開発しない」。
2. Claude / Codex は `docs/DEVELOPMENT_BOARD.md` の当該行と本ファイルを正とする。
3. 管理表・引き継ぎに個人情報、入力された健康情報、個人を特定できる情報は書いていない。
...
日付: 2026-09-14
担当チャット: 日次自動開発（親鍵入れ替え手順の文書化）

## 今回実装したタスク

暗号化の親鍵を入れ替える手順が未文書化だった。暗号の版管理と入れ替え中の復号テストは既存。足りなかったのは運用手順と、全件を包み直す管理用コマンド。

### 何をしたか

1. `docs/account-cloud-storage-decision.md` 7.4節に、新しい版の作成 → 旧版を残したまま現行を足す → ownerロールで包み直し → 旧版の失効 → セッション失効、の順と担当を書いた。
2. `src/lib/server/user-data-key-rotation.ts` で包み直し計画と件数だけの報告文を切り出した。所有者ID・暗号文・包んだ鍵は報告に出さない。
...
- `pnpm lint`: 成功（警告・エラーなし）
- `pnpm test`: 成功（73 files / 764 tests）
- `pnpm build`: 成功

### 次にやること

1. **再認証だけを行う導線。** 設定画面はあるが、10分を過ぎると `/cloud-login` へ飛ばし、ログイン済みだと「ログイン済みです」とログアウトしか出ない。記録を保ったまま6桁コードだけやり直す受け皿が無い。
2. YC-UX-STORAGE-NOTICES は PR #64 が未マージ。再実装しない。
3. YC-UX-NORMAL-ENTRY は運営者の移行判断が先。
4. YC-MON-REQUIRED-CHECKS は担当者の GitHub 設定が必要。
5. 認証メールのキャリア到達確認、Managed Better Auth のGA再確認。

### 引き継ぎ事項

1. **包み直しは本番データで練習しない。** ダミーデータ専用のDBで行う。この実行では本番DBへ接続していない。
2. **`USER_DATA_DATABASE_URL`（`yorucare_app`）で包み直しを走らせない。** RLSで0件に見え、旧版を外す判断を誤る。
3. **旧版を `USER_DATA_KEK` から外すのは、包み直し後に現行版以外が0件になってから。**
4. 親鍵・接続文字列・所有者IDはPRやチャットに貼らない。
...
| Gmail以外のメール事業者への6桁コード到達 | **Outlook（Microsoft）で到達を確認** |
...
### PR #63 の自動レビュー対応（2026-09-13）

PRを出したところ、`chatgpt-codex-connector` から5件（P1×3・P2×2）の指摘が付いた。**5件とも実在する不具合だったため、すべて直した。**

1. **（P1）自動送信が重なり、古い内容が新しい世代として残る** — `backupAfterSave` が結果を待たない送信を始めるため、続けて保存すると送信が重なる。先に作った古い内容があとから届くと、そちらが最新として残り、復元したときに保存したはずの変更が消える。送信を必ず1件ずつにし、送信中の依頼は「あとで1回」とだけ覚えて、終わってから最新の内容を作り直して送るようにした（何件たまっても送り直しは1回）。
2. **（P1）記録以外の変更が預け直されない** — 預けるのは記録全体のまるごと1件なのに、預け直しは `saveRecord` のときだけだった。**消したはずの記録がクラウドに残り、復元で戻ってきてしまう。** `src/lib/storage.ts` のすべての変更経路（記録の削除・全削除、できること・やらないことの追加／変更／削除、復職日）と、アプリ起動時（`app-shell.tsx`）から預け直すようにした。起動時の送り直しは、コードのコメントが以前から「次の保存かアプリ起動のときに送り直す」と書いていながら実装されていなかった。
3. **（P1）戻したあと、預け直しが始まっていないのに「預ける端末にしました」と伝える** — 「クラウドから戻す」は預けることへの同意ではないため、戻しただけでは以後の変更は送られない。文面を「続けて預けるには、下の『この内容を預ける』を押してください」に変えた。
4. **（P2）端末の登録に失敗しても成功として終わる** — `claimThisDevice()` の結果を見ていなかった。失敗時は専用の案内を出す。
5. **（P2）記録が0件だと、「できること」等しか無い人が何も預けられない** — 「預ける」ボタンの判定を `isEmptyPayload` とそろえた。

回帰確認として、**5件とも修正前のコードで新設テストが実際に落ちる**ことを確認したうえで直した。`pnpm lint`・`pnpm test`（750件）・`pnpm build` はすべて成功。
...
## 直前のmainの引き継ぎ（PR #62、2026-09-12）からの申し送り

このブランチの作業中にmainへ入った変更。統合（マージ）済み。

- **ログイン確認のコード送信で、許可リスト外と送信失敗の案内を分けた。** 認証中継APIが許可リスト拒否のときだけ `code: "email_not_allowed"` を返し（本文サイズ超過の403には付けない）、画面はそのトークンを見て「入力を確かめてください」と「時間をおいてもう一度」を出し分ける。Neon Authのクライアントは未知の `code` を403用の符号へ置き換えるため、判定は `code` と `message` の両方を見る。
- **PR #51 / #52 はマージせず閉じること。** 同じ課題の古い候補で、2026-09-10時点の画面（`getSession()` 直呼び）が前提になっており、PR #61 のセッション判定と衝突する。


---

## 追記（2026-09-14）: クラウド保存を本番へ出した（第1段階）

PR #63（実装とPreview確認）とPR #65（説明・同意の方針、本番手順）を経て、**本番の入口を開けた**。

- **第1段階（運営者だけの試運転）は完了。** `https://yorucare.vercel.app` で、カード表示・ログイン・預ける・復元・退会まで確認した。許可リストは運営者のアドレスだけで、**参加者のデータは1件も預かっていない**
- 手順と結果は `docs/account-cloud-storage-decision.md` 11.3節
- 説明・同意の方針と、参加者へ配れる説明文の草案は `docs/cloud-backup-consent-draft.md`

**次に参加者へ広げるときの条件（第2段階）**:

1. ③個人情報の確認（要配慮個人情報に当たるか、国外移転の扱い）。質問文は `cloud-backup-consent-draft.md` 6.2節にそのまま使える形で置いてある
2. 参加者向け説明文の確定とGoogleフォームの作成
3. `yorucare_app` ロールのパスワード再作成
4. そのうえで `USER_DATA_ALLOWED_EMAILS` へ参加者のアドレスを追加する。**許可リストが最後の安全弁**

研究に関する確認は、5か月プログラムの実施時に行う（その時点で同意を得て、それ以降のデータから集計する）。

**開発せずに記録だけ残した改善タスク2件**（`docs/DEVELOPMENT_BOARD.md`）:

- 同意の日時と文面の版が残らない（現状は端末内に `granted` / `denied` のみ）
- 運営から利用者へ知らせる手段がない（提供終了の1か月前予告を説明文で約束したが、その仕組みがない）
