...
### 1（ログアウト失敗）についての現時点の見立て

**原因はまだ確定していない。** ただし、2の原因（固定IDが先に見られていた）が分かったことで、次の見立てが立つ。

- 画面の「ログイン済みです」は固定ID由来だった可能性がある。その場合、**本物のBetter Authのセッションは実は成立しておらず**（またはすでに切れており）、サインアウトが本当に失敗する（401など）のは自然な結果になる。「この内容を預ける」「復元」が通っていたのも、固定IDの所有者IDで動いていたためと説明できる。
- 今回の修正後は、この取り違えが画面に出る。`/cloud-login` に「検証用の固定IDで表示しています」が出るかどうかで、どちらの話かがすぐ分かる。

### 検証結果

- `pnpm lint`: 成功（警告・エラーなし）
- `pnpm test`: 成功（68 test files / 718 tests）
- `pnpm build`: 成功
- 回帰確認: 新設テストのうち、`cloud-session` の3件と認証中継の1件は、**修正前のコードで実際に落ちることを確認**したうえで直した

### 次にやること（Preview環境での確認、ユーザー操作）

1. **Vercelの環境変数から `USER_DATA_DEV_OWNER_ID` を消す（Preview）。** 本物のログインを確かめたいあいだは設定しない。ダミーデータを認証なしで通したいときだけ戻す。
2. **`USER_DATA_ALLOWED_EMAILS` に `gmpm1001@gmail.com` だけを入れて再デプロイし**、`/cloud-login` でログインし直す。画面に「検証用の固定IDで表示しています」が出ないことを確かめる（出るなら1がまだ効いていない）。
3. **「ログアウトする」を押す。** 失敗するなら、画面に出る「（詳細: …）」の番号をそのまま共有する。開発者ツールは不要。
4. **許可リストを空にして再デプロイし**、`/cloud-login` をスーパーリロード（Ctrl+Shift+R / Cmd+Shift+R）して「このメールアドレスはクラウド保存の利用対象に登録されていません」が出ることを確かめる。
