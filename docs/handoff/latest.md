...
- **すべての指標を Day 0〜Day 27 の同じ窓で数える。** 窓を固定したので、いったん分母へ入った端末・ブラウザの値は、集計日を後ろへずらしても変わらない。
- **分母へ入れるのは Day 27 が終わってから**（集計日が Day 28 以降）。**ここは自動レビューの指摘で直した**（下記）。従来は Day 27 当日から数えていたため、その日のうちに届いた保存で報告済みの値が後から動いた。
...
- テスト: `src/lib/analytics-retention.test.ts`（+8件）、`src/lib/analytics-retention-report.test.ts`（**新規** 6件）、`src/lib/analytics.test.ts`（**新規** 3件）

## 検証結果

- `pnpm lint`: 成功（ESLint の警告・エラーなし）
- `pnpm test`: 成功（**45 test files / 447 tests**。枝を作った時点の `origin/main`（`5c90e1d`）は 43 files / 430 tests だったので、今回の追加は17件）
...
- **テストが本当に効くことを確かめた。** 中断の判定から「窓の終わりまで戻らなかった場合」を外すと6件落ちる。分母の条件を `< 27` に戻すと1件落ちる。`getRecentSaveStreakHint` を直す前の式に戻すと1件落ちる。

## 自動レビュー指摘

- **PR #29: 1件（P2）。対応した。** 「Day 27 が終わるまで対象に入れないこと」。集計日が Day 0 から数えて offset 27（＝ Day 27 当日）の時点で、その端末・ブラウザを分母へ入れていたため、**同じ日のうちに保存が届くと、報告済みの Week 4 継続率・週あたり記録日数・中断後再開率が後から変わる**という指摘。
- **自分で再現して確かめたところ、指摘は正しかった。** day0 を 2026-06-01 とし、集計日 2026-06-28（Day 27 当日）で Week 4 継続率は 0%。同じ日に保存が1件届いた状態で数え直すと 100% になる。`docs/phase2-plan.md` 4節の「Day 0 から4週間を観測できた install」を満たしておらず、**このPR自身が掲げた「集計日を後ろへずらしても値は変わらない」にも反していた。**
- **これは今回入れた退行ではなく、元からあった条件（`< 27`）**だが、読み出す経路を今回作ったことで「報告した数値が後から動く」形で表に出る。条件を `< OBSERVATION_DAYS`（28以上を要求）へ直し、境界の単体テストを足した。
- 追加したテストは、**直す前の条件に戻すと落ちる**ことを確かめてある（`expected 1 to be +0`）。
- 指摘はPR作成の約4分後（CI完了の約2分半後）に付いた。**CIが緑になった時点では0件だった。**
- 前のチャットから引き継いだ未対応の指摘はない（PR #28 は別チャットが扱った）。
