...
日付: 2026-09-15
担当チャット: メニュー改善・要望フォームの記録のみ（実装しない）

## 今回実装したタスク

なし。開発管理表へ未着手課題を1行追加しただけ。アプリのコードは変更していない。

課題名: メニューからアプリの改善・要望を送れない（YC-UX-FEEDBACK-FORM）

運営からの依頼は、ヘッダーメニューへ「アプリの改善・要望」用Googleフォームへ飛べる項目を足すこと。このチャットでは実装せず、Claude と Codex が読めるリポジトリ文書へ残して終了する。

## 変更ファイル

- `docs/DEVELOPMENT_BOARD.md`: 課題行を `未着手` で追加。実行履歴に記録のみである旨を書いた
- `docs/handoff/latest.md`: 本ファイル

## 検証結果

- 文書の記録のみ。`pnpm lint` / `pnpm test` / `pnpm build` は対象外
...
1. **YC-UX-FEEDBACK-FORM（今回記録した課題）。** 要件は下に揃えてある。運営が次チャットへ実装を頼む想定。他の未着手より優先するかは運営判断。
2. 再認証だけを行う導線、YC-UX-STORAGE-NOTICES、YC-UX-NORMAL-ENTRY など既存の未着手は優先順位を変えていない。

## 引き継ぎ事項・注意点

1. **この依頼では実装しない。** 次に着手するチャットがコードを変える。
2. **置き場所。** メニューは `src/components/layout/app-menu.tsx`。いまの項目は「相談先」と「カスタム入力」の2つ。相談先は先頭のまま。新しい項目はその後ろ（カスタム入力の下）に足す。
3. **開き方。** いまの `MenuItem` は `button`。フォームはアプリ内ダイアログではなく外部URLなので、相談先の公式サイトと同じく `<a target="_blank" rel="noopener noreferrer">` で開く。見た目は既存のメニュー項目に合わせる。
4. **文言。** `src/lib/copy.ts` の `COPY.menu` にラベル・説明を足す。メニュー説明（いまは「相談先と、記録画面に出す項目の設定を開けます。」）も項目追加に合わせて直す。診断・治療・危機判定に読める表現は使わない。
5. **リンク先（公開用 viewform。所有者を示すクエリは付けない）。**
   `https://docs.google.com/forms/d/e/1FAIpQLSdIF5ht-nRhLBtjQiLUrkNrxOs2cgl1HZP3FrL7Sx7hK1CKNg/viewform?usp=sharing`
6. **送らないもの。** 記録、端末内の設定、識別子をアプリからフォームへ付けない。クエリで記録内容を渡さない。本人がフォームを開いて書いた内容だけがGoogleフォーム側に届く。
7. **テスト。** `src/components/layout/app-menu.test.tsx` に、メニューを開くと新項目が出ること、`href` が上のURLであること、相談先・カスタム入力が残ることを足す。
8. **範囲。** メニュー項目の追加だけ。相談先の並び、カスタム入力、記録画面、クラウド保存には手を出さない。
