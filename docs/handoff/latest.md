...
1. **このブランチ（`claude/focused-cerf-fezd9z`）をPreviewへデプロイする。** 新しいURLになるので、**Neon ConsoleのDomainsへ登録**する。このとき、デプロイごとに変わるハッシュ付きURLではなく、ブランチ名入りの固定URL（`yorucare-git-claude-focused-cerf-fezd9z-….vercel.app`）を登録すると、以後の追加が要らなくなる。
2. `/cloud-login` を開き、下部の「検証用の情報」を開く。**「いま動いているコミット」が最新のものか**を確かめる（ここが古ければ、見ているURLが古いデプロイ＝2の原因が確定する）。
3. ログインして「ログアウトする」を押す。**415の修正が効いていれば成功するはず。** 失敗したら、画面に出る「（詳細: …）」の番号をそのまま共有する（開発者ツールは不要）。
4. `USER_DATA_ALLOWED_EMAILS` を空にして再デプロイし、**新しいURLで**「検証用の情報」の「許可リストの件数」が0件になっていることを確かめてから、「このメールアドレスはクラウド保存の利用対象に登録されていません」が出ることを確認する。
5. `NEON_AUTH_BASE_URL` にVercelの「Needs Attention」の印が付いている。何の警告かを確認する（「検証用の情報」の「認証基盤」が「届く（200）」なら実害は無い）。
