...
1. **許可リスト外の確認（残りの1件）。** `USER_DATA_ALLOWED_EMAILS` を `gmpm1001+notallowed@gmail.com` だけに変えて再デプロイし、**「検証用の情報」の「許可リストの件数」が1件のまま／中身が入れ替わったこと**を確かめたうえで、`gmpm1001@gmail.com` でログインを試み、「このメールアドレスはクラウド保存の利用対象に登録されていません」が出ることを確認する。確認後は元に戻す。
2. **記録タブの設定画面の「退会」をやり直す。** 必ず**ブランチ名入りの固定URL**
   （`https://yorucare-git-claude-focused-cerf-fezd9z-kuma-0509s-projects.vercel.app`）で開き、
   「検証用の情報」の「この画面のURL」がDomainsの登録と一致していることを確かめてから実行する。
   預ける→最終預け日時→復元→停止までは2026-09-13に確認済み。
3. **別端末での復元確認**（未実施）。別端末でも**同じ固定URL**を開くこと。
   `NEXT_PUBLIC_CLOUD_BACKUP_ENABLED` はデプロイに焼き付くため、別のURL
   （本番URLなど）を開くとクラウド保存の画面そのものが出ない。
4. **参加者が実際に使うメール事業者での到達確認**（現在Gmail1アカウントのみ）。
5. `NEON_AUTH_BASE_URL` にVercelの「Needs Attention」の印が付いている。何の警告かを確認する（「検証用の情報」の「認証基盤」が「届く」と出ており、実害は確認されていない）。
6. すべて終わったら、ユーザーの合図でPRを開く。
