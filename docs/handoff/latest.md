...
6. **Neon Consoleでの作業手順（ユーザー作業分）**
   ...
   | 許可リスト外のアドレスでは弾かれる | ⚠️ **APIは弾いた。画面は弾いていない。** 許可リストに無いアドレス（同じ受信箱に届く別表記）で6桁コードまで通したところ、`/cloud-login` は「ログイン済みです」を表示した。一方 `GET /api/cloud/snapshot` は **401 `{"ok":false}`**。`getCloudSession` の `isEmailAllowed` が効いており、**記録の読み書きは守られている**。ただし確認画面はBetter Authのセッション有無しか見ておらず、許可リストの結果を反映していない（下記の注意点11） |
   ...
   「許可リスト外の拒否」は、同じ受信箱で受け取れる別表記のアドレス（`ユーザー名+任意の文字@gmail.com` など）で試せる。新しいメールアドレスを用意する必要はない。**この検証で作られた認証アカウントは、Console の Users タブから削除しておくこと**（新規登録を止められない以上、放置すると不要なアカウントが残る）。

   なお、**別端末での復元確認は未実施**のまま残っている。
...
11. **`/cloud-login` の「ログイン済みです」は、許可リストの結果を反映していない（2026-09-11 実測）。** この画面はBetter Authのセッション有無だけを見ており、`getCloudSession` の `isEmailAllowed` を通っていない。そのため許可リストに無い人でも「ログイン済みです」と表示される。記録APIは401で弾くので**データは守られている**が、参加者から見ると「ログインできたのに預けられない」という分かりにくい状態になる。手順書6.7の「許可リストに含まれていないメールアドレスでは、コード検証後もログインできない」という記述も、実装と食い違っている（ログイン自体はできる）。公開前に、この画面が「クラウド保存を使える状態か」を表示するよう直し、手順書の文面も合わせること。
